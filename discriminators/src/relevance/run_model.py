...
class TextDataset(Dataset):
    ...
    def __getitem__(self, i):
        return {
            ...
            'inputs_type': torch.tensor(self.inputs_type[i], dtype=torch.long), 
            'label': self.labels[i]
        }
...
def load_and_cache_examples(args, tokenizer, evaluate=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        inputs = tokenizer.batch_encode_plus(inputs, add_special_tokens=True, max_length=block_size)
        data = [inputs['input_ids'], inputs['token_type_ids'], labels]
...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    def collate(examples: List[Dict]):
        ...
        return {
            ...
            'inputs_type': pad_sequence(inputs_type, batch_first=True, padding_value=tokenizer.pad_token_id), 
            'label': labels
        }
...
def evaluate(args, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, prefix="") -> Dict:
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            n_precision[0] += torch.sum(n_correct_vec * prediction.eq(1).float())
    ...
    fpr, tpr, thresholds = metrics.roc_curve(label_list, score_list, pos_label=1)
    ...
    result = {
        ...
        'recall': n_recall[0] / n_recall[1], 'precision': n_precision[0] / n_precision[1]
    }
